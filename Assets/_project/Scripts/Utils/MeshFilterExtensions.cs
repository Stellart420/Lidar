...
{
    public static void GenerateUV(this MeshFilter meshFilter, Camera camera, (int,int) textureSize)
    {
        Debug.Log($"Generate UV.{textureSize.Item1}/{textureSize.Item2}");
        Mesh mesh = new Mesh();
        ...
        Vector2[] textureCoordinates = CalcTextureCoordinates(mesh, meshFilter.transform.localToWorldMatrix, camera, textureSize);
    ...
    private static Vector2[] CalcTextureCoordinates(Mesh geometry, Matrix4x4 modelMatrix, Camera camera, (int, int) textureSize)
    {
